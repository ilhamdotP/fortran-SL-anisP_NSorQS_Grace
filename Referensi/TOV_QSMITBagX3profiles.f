...
c      OPEN (unit=2,STATUS='unknown',FILE='CatatQS.dat')
      OPEN (unit=3,STATUS='unknown',FILE='RadmassQS_aniso.dat')
C       OPEN (unit=1,STATUS='unknown',FILE='Profiles_anisoQS.dat')
...
C       WRITE(1,*)(XP/1.D3),Y(1),Y(2),Y(3),(Y(1)-PRT)     ! print profil
...
c      a4T=0.7D0
      ...
      GAMMA=8.D0*PI/(3.D0*ms**2*DSQRT(a4T)*HC**(3/2))
...
C     FED=3.D0*P0+4.0*BagC
      ...
      GAMMA=8.D0*PI/(3.D0*ms**2*DSQRT(a4)*HC**(3/2))
      KAPPA=3.D0/(1.D0-1.D0/(a4))
      DEPS=1.5D0*DSQRT(3.D0)*ALPHA*DSQRT(3.D0*ALPHA**2+4.D0*(BagC+PTD))
     &     +4.5D0*ALPHA**2
      PTD=P0
