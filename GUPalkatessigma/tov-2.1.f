...
CCC     write parameters to filename WITHOUT YDY
       ...
       FILENAME1 = 'profilmod_CT=' // trim(adjustl(FILECT)) //'.dat'
       FILENAME2 = 'radmassmod_CT=' // trim(adjustl(FILECT)) //'.dat'
...
C       WRITE(*,*)IL,(XP/1.D3),PRESS,MASST,SIGMA
...
C         INCLUDE "EOS.f"
      ...
      SUBROUTINE FUNCT(EK,J,YA,XA,H,PCC)
C     *********************************************************
...
C-----------------------------------------------------------------------
      ...
      DSDP=(SIG(x4,PCC)-8.D0*SIG(x2,PCC)
     & +8.D0*SIG(x1,PCC)-SIG(x3,PCC))/(12.D0*h)
...
c---------------------------------------------------------------------
     ...
     -  9.644098826004996D-17*P0**6
      ...
      IF (P0.EQ.PCC) THEN
      SIG = SIG -(
     -  0.00893216184253268D0 + 0.0066138230853899135D0*PCC - 
     -  0.00002547856239766327D0*PCC**2 + 1.227701424673366D-7*PCC**3 - 
     -  2.525768285081147D-10*PCC**4 + 2.5092894933230487D-13*PCC**5 - 
     -  9.644098826004996D-17*PCC**6  )
      ENDIF
